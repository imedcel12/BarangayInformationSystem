...
<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal">
  ...
  <form method="post" action="{{url('create_certificate_indigencies')}}">
    @csrf
  <!-- Modal -->
...
<div class="">
    <div class="col-md-12">
        <h1 class="mt-1" style="font-weight: 400; font-size: 35px;">Certificate of Indigency</h1>
        <hr>
        <table class="table table-striped table">
            ...
            <tbody class="text-center">
                ...
                @foreach ( $certificate_indigencies as $item )
                    <tr class="text-dark">
                      <td><button class="btn btn-sm btn-info">
                        <a href="{{url('export_certificate_indigencies/'.$item->id)}}" class="btn btn-sm text-light">Print</a>
                    </button></td>
                        <td>{{$item->name}}</td>
                        <td>{{$item->address}}</td>
                        ...
                        <td><span> <a href="{{url('edit_certificate_indigencies/'.$item->id)}}" class="btn btn-warning btn-sm text-light">Edit</a></span>
                         <span>
                            <a href="{{url('delete_certificate_indigencies/'.$item->id)}}" class="btn btn-danger btn-sm text-light">Delete</a>
                        </span></td>
                    </tr>
                @endforeach
